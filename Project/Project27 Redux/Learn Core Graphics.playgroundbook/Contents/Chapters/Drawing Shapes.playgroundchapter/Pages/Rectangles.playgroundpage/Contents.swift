//#-hidden-code
import UIKit
let imageView = UIImageView()
//#-end-hidden-code
//: # Welcome to Core Graphics!
//: Core Graphics is Apple's high-performance drawing framework, and you can use it to draw shapes, text, images, gradients, and more.
//:
//: In this playground you'll learn the fundamentals of working with Core Graphics, starting with the basics: how to draw rectangles.
//:
//: The code below creates a 1000x1000 image with a 600x600 blue box in its center, then displays it in the image view you can see on the right.
//:
//: - Experiment: Can you write some code to draw a red box on top of the blue one, making it 200x200 and centered like the blue one?
let rect = CGRect(x: 0, y: 0, width: 1000, height: 1000)
let renderer = UIGraphicsImageRenderer(bounds: rect)

let rendered = renderer.image { ctx in
    UIColor.blue.setFill()
    ctx.cgContext.fill(CGRect(x: 200, y: 200, width: 600, height: 600))
    //#-editable-code Tap to add your drawing instructions
    //#-end-editable-code
}

imageView.image = rendered
//#-hidden-code
sendValue(rendered)
//#-end-hidden-code
